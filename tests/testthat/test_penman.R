...
test_that('No Tmin provided', {
  expect_error(penman(Tmax=TMAX, U2=AWND, tsun=TSUN, lat=37.6475, z=402.6,
                      na.rm=TRUE),
               'argument "Tmin" is missing, with no default')
})

test_that('No Tmax provided', {
  expect_error(penman(TMIN, U2=AWND, tsun=TSUN, lat=37.6475, z=402.6,
                      na.rm=TRUE),
...
test_that('P0 without z provided', {
  expect_error(penman(TMIN, TMAX, AWND, tsun=TSUN, lat=37.6475, P0=TSUN,
                      na.rm=TRUE),
               'One of `P`, the pair `P0` and `z`, or `z` must be provided.')
})

test_that('Unconformed crop provided', {
  expect_error(penman(TMIN, TMAX, AWND, tsun=TSUN, lat=37.6475, z=402.6,
                      crop='corn', na.rm=TRUE),
               'Argument `crop` must be one of `short` or `tall`.')
})

test_that('NAs in Tmin', {
  expect_error(penman(c(NA,TMIN), c(1,TMAX), c(1,AWND), tsun=c(1,TSUN),
                      lat=37.6475, z=402.6, na.rm=FALSE),
               'Data must not contain NA values if argument `na.rm` is set to FALSE.')
})

test_that('NAs in Tmax', {
  expect_error(penman(c(1,TMIN), c(NA,TMAX), c(1,TMIN), tsun=c(1,TMIN),
                      lat=37.6475, z=402.6, na.rm=FALSE),
               '`Tmin`, `Tmax` and `U2` must not contain NA values if argument `na.rm` is set to FALSE.')
})

test_that('NAs in U2', {
  expect_error(penman(c(1,TMIN), c(1,TMAX), c(NA,TMIN), tsun=c(1,TMIN),
                      lat=37.6475, z=402.6, na.rm=FALSE),
               '`Tmin`, `Tmax` and `U2` must not contain NA values if argument `na.rm` is set to FALSE.')
})

test_that('NAs in Tsun', {
  expect_error(penman(c(1,TMIN), c(1,TMAX), c(1,TMIN), tsun=c(NA,TMIN),
                      lat=37.6475, z=402.6, na.rm=FALSE),
               'Data must not contain NA values if argument `na.rm` is set to FALSE.')
})

test_that('NAs in z', {
  expect_error(penman(c(1,TMIN), c(1,TMAX), c(1,TMIN), tsun=c(1,TMIN),
...
test_that('Tmax incorrect length', {
  expect_error(penman(TMIN, TMAX[-1], AWND, tsun=TSUN, lat=37.6475, z=402.6,
                      na.rm=TRUE),
               '`Tmin` and `Tmax` cannot have different lengths.')
})

test_that('U2 incorrect length', {
  expect_error(penman(TMIN, TMAX, AWND[-1], tsun=TSUN, lat=37.6475, z=402.6,
                      na.rm=TRUE),
               '`U2` has incorrect length.')
})

test_that('Ra incorrect length', {
  expect_error(penman(TMIN, TMAX, AWND, Ra=TMIN[-1], lat=37.6475, z=402.6,
                      na.rm=TRUE),
               '`Ra` has incorrect length.')
})

test_that('Rs incorrect length', {
  expect_error(penman(TMIN, TMAX, AWND, Rs=TMIN[-1], lat=37.6475, z=402.6,
                      na.rm=TRUE),
               '`Rs` has incorrect length.')
})

test_that('tsun incorrect length', {
  expect_error(penman(TMIN, TMAX, AWND, tsun=TSUN[-1], lat=37.6475, z=402.6,
                      na.rm=TRUE),
...
test_that('ed incorrect length', {
  expect_error(penman(TMIN, TMAX, AWND, tsun=TSUN, ed=TMIN[-1], lat=37.6475,
                      z=402.6, na.rm=TRUE),
               '`ed` has incorrect length.')
})

test_that('Tdew incorrect length', {
  expect_error(penman(TMIN, TMAX, AWND, tsun=TSUN, Tdew=TMIN[-1], lat=37.6475,
                      z=402.6, na.rm=TRUE),
               '`Tdew` has incorrect length.')
})

test_that('RH incorrect length', {
  expect_error(penman(TMIN, TMAX, AWND, tsun=TSUN, RH=TMIN[-1], lat=37.6475,
                      z=402.6, na.rm=TRUE),
...
test_that('P incorrect length', {
  expect_error(penman(TMIN, TMAX, AWND, tsun=TSUN, lat=37.6475, P=TMIN[-1],
                      na.rm=TRUE),
               '`P` has incorrect length.')
})

test_that('P0 incorrect length', {
  expect_error(penman(TMIN, TMAX, AWND, tsun=TSUN, lat=37.6475, P0=TMIN[-1],
                      z=402.6, na.rm=TRUE),
...
test_that('example with different CO2', {
  expect_equal(
    ...
    penman(TMIN, TMAX, AWND, tsun=TSUN, lat=37.6475, z=402.6, CO2=450,
           na.rm=TRUE)
  )
})
...
test_that('example with increasing CO2', {
  expect_equal(
    ...
    penman(TMIN, TMAX, AWND, tsun=TSUN, lat=37.6475, z=402.6, CO2=co2,
           na.rm=TRUE)
  )
})
